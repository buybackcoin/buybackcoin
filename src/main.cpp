...
uint256 hashGenesisBlock("0x00000acab9a3dec1d738e18a2c9b081f6a4a6c53a910b1fb5299cdb47f756566"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // buybackcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "buybackcoin Signed Message:\n";
...
{
    // buybackcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    if(nHeight >= 5465) {
        if((nHeight >= 17000 && dDiff > 75) || nHeight >= 24000) { // GPU/ASIC difficulty calc
            ...
            if (nSubsidy > 25) nSubsidy = 9;
            if (nSubsidy < 5) nSubsidy = 5;
            if (nSubsidy >= 5 && nSubsidy <= 25) nSubsidy = 7;
        } else { // CPU mining calc
            nSubsidy = (11111.0 / (pow((dDiff+51.0)/6.0,2.0)));
            if (nSubsidy > 500) nSubsidy = 9;
            if (nSubsidy < 25) nSubsidy = 5;
            if (nSubsidy >= 25 && nSubsidy <= 500) nSubsidy = 8;
        }
    } else if (nHeight == 1) 
    	{
    		nSubsidy = 400000000;
        ...
        if (nSubsidy > 500) nSubsidy = 10;
        if (nSubsidy < 1) nSubsidy = 1;
        if (nSubsidy >= 1 && nSubsidy <= 500) nSubsidy = 7;
    } 

    // printf("height %u diff %4.2f reward %i \n", nHeight, dDiff, nSubsidy);
    nSubsidy *= COIN;

    // yearly decline of production by 7% per year--262800=365 * 720
    for(int i = 262800; i <= nHeight; i += 262800) nSubsidy *= 0.93;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // buybackcoin: 1 day
static const int64 nTargetSpacing =  2 * 60; // buybackcoin: 2.5 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 720
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // buybackcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, buybackcoin - DarkGravity v2, written by Evan Duffield - evan@buybackcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, buybackcoin - DarkGravity v3, written by Evan Duffield - evan@buybackcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // buybackcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // buybackcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "NY Times 2014-06-12 haha by Imprisoned Leaders of Egyptian Revolt Rejected";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402558309; //2014-6-12 pm 3:31:49
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 30446763;
        ...
        assert(block.hashMerkleRoot == uint256("0x9fcf80b0858e07dc006d8093578cc2e218a1dc58fdfc74130aa8144020322845"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("buybackcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("buybackcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static buybackcoinMiner(CWallet *pwallet)
{
    printf("buybackcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("buybackcoin-miner");
    ...
    try { loop {
        ...
        printf("Running buybackcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("buybackcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&buybackcoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - buybackcoin **********
    Copyright 2014, buybackcoin Developers 
        eduffield - evan@buybackcoin.io
        InternetApe - kyle@buybackcoin.io
*/
