...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "buybackcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"202.107.209.88", "202.107.209.88"}, //Evan's seed node
    {"202.107.209.87", "202.107.209.87"},
    {NULL, NULL},
};
...
// '{:02X}{:02X}{:02X}{:02X}'.format(*map(int, "121.32.150.196".split('.')))

unsigned int pnSeed[] =
{
    0xca6bd158, 0xca6bd157
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. buybackcoin is probably already running."), addrBind.ToString().c_str());
        else
